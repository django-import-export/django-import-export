...
- Paginate the admin import preview using Django's ``Paginator`` so admins can page through every dry-run row instead of only the first slice. Page size is controlled by the new ``IMPORT_EXPORT_PREVIEW_PAGE_SIZE`` setting (default ``100``) (`2148 <https://github.com/django-import-export/django-import-export/issues/2148>`_)
